...
public class DemoState extends AbstractState implements State {
	...
	public void update(Game g, float elapsed) {
		...
		if (scoreObject != null) {
			if (objects.containsKey("player") && objects.get("player").attributes.containsKey("score")) {
				double score = (Double) objects.get("player").attributes.get("score");
				scoreObject.text = String.format("%05d", score);
			}
		}
	}
